...
New YC Fit reports lock the browser-generated score and enqueue a Vercel Workflow whose first step atomically claims the report, researches the five closest public YC-company neighbors, and then drafts a citation-backed coaching dossier. Configure `AI_REPORT_MODEL` independently from chat/profile extraction. Durable public research requires `FIRECRAWL_API_KEY`, `FIRECRAWL_WEBHOOK_SECRET`, and a public HTTPS `NEXT_PUBLIC_SITE_URL`; without that complete contract the report uses its explicit no-research fallback instead of starting an unrecoverable paid job. Workflow owns Firecrawl job recovery, the ten-minute deadline, drafting, and terminal Turso status updates. The report page performs read-only polling of that saved status, so leaving or refreshing it cannot restart or advance research. Signed callbacks update job rows promptly, while Workflow polling remains the recovery path when a callback is delayed.
...
YC Fit reports and YC company reports run asynchronously through Vercel Workflow. The authenticated request creates or claims the owner-scoped report row, schedules the durable research run, and returns its private progress URL. Firecrawl retrieval and AI synthesis execute in Workflow steps, while Turso remains the user-facing status and result source of truth. The application-report page only polls Turso; the company-report page does the same before running the existing versioned semantic map in the browser once its cited draft reaches `mapping`. Wrap `next.config.ts` with `withWorkflow()` and keep the `workflow` package installed so local development and Vercel deployments register the workflow and step routes.
