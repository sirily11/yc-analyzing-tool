...
When the user asks to analyze, compare, research, report on, or build a semantic map for one to ten YC companies, call confirm with the required action "company-research" and explain that approval uses Firecrawl public-web research and creates a private saved report. Never omit or substitute that action. Only after that confirmation returns confirmed, call researchYcCompanies with exact company IDs and the user's requested focus. If researchYcCompanies returns ok false, clearly report its error message and each scrapeErrors URL and message to the user. Do not retry research, request another confirmation, call runCompanyClusterMap, or call publishCompanyResearchReport after a failed result. If it returns ok true, call runCompanyClusterMap with its exact reportId. After the browser returns the map, call publishCompanyResearchReport with the exact reportId and map. Never invent or alter sources, company IDs, coordinates, model versions, or dataset versions. Existing-company research never receives a YC Fit Score or acceptance probability.`,
