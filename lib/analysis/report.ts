...
export function buildReportDocument(profile: ApplicationProfile, prediction: PredictionResult, companies: YcCompany[]): ReportDocument {
  ...
  const gaps = profile.missingFields.length ? profile.missingFields.map((field) => `The plan needs more decision-grade evidence for ${field}.`) : ["The core application fields are present; the remaining work is specificity and compression."];
  ...
  return {
    ...
    gaps,
    recommendations,
    methodology: "The score is the percentile position of an accepted-company profile against a versioned 2022–2026 YTD public dataset. Text is categorized into a fixed schema, vectorized locally, and compared with reference-company latent features. It is not trained on rejected applications.",
    disclaimer,
  };
}
